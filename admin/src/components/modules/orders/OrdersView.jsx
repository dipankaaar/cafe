...
import { normalizeOrderStatus, getStatusLabel, getStatusBadgeVariant } from '../../../utils/orderStatus';
...
export default function OrdersView() {
  ...
  const getStatusBadge = (status) => {
    ...
    switch (s) {
      case 'placed':
      case 'accepted':
        return <Badge variant="info" dot>New Order</Badge>;
      case 'brewing':
      ...
      case 'ready':
        return <Badge variant="primary" dot>Ready</Badge>;
      case 'completed':
      ...
      default:
        return <Badge variant="info" dot>{getStatusLabel(status)}</Badge>;
    }
  };
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <select
                        ...
                        className="bg-gray-50 dark:bg-[#1a1a1a] text-xs font-semibold border border-gray-200 dark:border-gray-700 rounded-lg px-2.5 py-1 text-gray-900 dark:text-white outline-none cursor-pointer hover:border-[#DD5903] transition-colors"
                    ...
                    <td className="px-5 py-3.5 text-right space-x-1">
                      ...
                      {normalizeOrderStatus(order.status) !== 'cancelled' && normalizeOrderStatus(order.status) !== 'completed' && normalizeOrderStatus(order.status) !== 'delivered' && (
                        <button
        ...
        <Modal
          ...
          footer={
            <>
              ...
              {(normalizeOrderStatus(selectedOrder.status) === 'placed' || normalizeOrderStatus(selectedOrder.status) === 'accepted') && (
                <Button
                  variant="primary"
                  icon={ChefHat}
                  onClick={() => {
                    updateOrderStatus(selectedOrder.id, 'brewing');
              ...
              {normalizeOrderStatus(selectedOrder.status) === 'brewing' && (
                String(selectedOrder.orderType || '').toLowerCase() === 'delivery' ? (
                  <Button
                    variant="primary"
                    icon={Bike}
                    onClick={() => {
              ...
              {normalizeOrderStatus(selectedOrder.status) === 'out_for_delivery' && (
                <Button
                  variant="success"
                  icon={CheckCircle2}
                  onClick={() => {
              ...
              {normalizeOrderStatus(selectedOrder.status) === 'ready' && (
                <Button
                  variant="success"
                  icon={CheckCircle2}
                  onClick={() => {
