...
export default function ScanTablePage({ onNavigate, initialTableToken }) {
  ...
  // Camera initialization & QR scanning loop
  useEffect(() => {
    if (!activeTableToken) {
      startCamera();
    } else {
  ...
  useEffect(() => {
    if (!isCameraActive || !videoRef.current || activeTableToken) return;
        ...
        if (detector) {
          try {
            ...
            if (codes && codes.length > 0) {
              ...
              if (token && token.trim()) {
                handleSelectTableToken(token.trim());
                return;
              }
            }
          } catch (err) {
  ...
  }, [isCameraActive, activeTableToken]);
