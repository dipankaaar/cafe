...
export function runDatabaseSeeds(force = false) {
  ...
  if (result.count === 0 || force) {
    ...
    // 2. Branches (IGNORE: preserves manager/phone edits + created rows)
    const brStmt = db.prepare('INSERT OR IGNORE INTO branches (id, name, code, address, phone, email, manager_name, opening_hours, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    initialBranches.forEach((b) => {
      brStmt.run(b.id, b.name, b.code, sanitize(b.address, ''), sanitize(b.phone, ''), sanitize(b.email, ''), sanitize(b.managerName, ''), sanitize(b.openingHours, ''), b.isActive ? 1 : 0, new Date().toISOString());
    });
    // Backfill: every operational row without a branch belongs to the flagship store
    ['orders', 'tables_floor', 'reservations', 'expenses', 'inventory', 'staff', 'purchases'].forEach((t) => {
      try { db.prepare(`UPDATE ${t} SET branch_id = 'br-main' WHERE branch_id IS NULL`).run(); } catch (e) {}
    });

    // 2b. Categories (ON CONFLICT DO UPDATE — NOT "OR REPLACE": REPLACE deletes the
